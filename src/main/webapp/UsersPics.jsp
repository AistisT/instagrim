            ...
            <div class="col-md-10">
                <ul class="row">
                    ...
                    <%} else {
                        ...
                        while (iterator.hasNext()) {
                            Pic p = (Pic) iterator.next();%>
                    <li class="col-lg-3 col-md-3 col-sm-4 col-xs-5 text-center">  <figure><a href="${pageContext.request.contextPath}/Image/<%=p.getSUUID()%>" ><img src="${pageContext.request.contextPath}/Thumb/<%=p.getSUUID()%>"></a>
                            <figcaption> 
                                <form class="btn-group btn-group-sm" role="group" aria-label="Like" method="POST" action="${pageContext.request.contextPath}/Like">
                                    <button type="submit" <% if (p.getLiked() || !ownAcLog) {%>disabled <%}%> class="btn btn-success">Like</button>
                                    ...
                                    <input type="hidden" name="source" value="userpics">
                                ...
                                <form class="btn-group btn-group-sm" role="group" aria-label="Dislike"method="POST" action="${pageContext.request.contextPath}/Like">
                                    <button type="submit" <% if (p.getDisliked() || !ownAcLog) {%> disabled <%}%>class="btn btn-primary">Dislike</button>
                                    <input type="hidden" name="userName" value="<%=(String) request.getAttribute("user")%>">
                                    <input type="hidden" name="picid" value="<%=p.getSUUID()%>">
